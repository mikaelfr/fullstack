...
blogsRouter.post('/', async (request, response, next) => {
  ...
  try {
    ...
    await user.save()
    const populatedBlog = await Blog.findById(result._id).populate('user', { name: 1, username: 1, _id: 1 })
...
blogsRouter.put('/:id', async (request, response, next) => {
  ...
  try {
    const updated = await Blog.findByIdAndUpdate(request.params.id, blog, { new: true }).populate('user', { name: 1, username: 1, _id: 1 })
